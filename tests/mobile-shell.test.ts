...
describe("Flutter mobile shell scaffold", () => {
  ...
  it("loads the configured web app URL", () => {
    ...
    expect(main).toContain("encryptedSharedPreferences: true");
    ...
    expect(main).toContain("Uri.file(file.path).toString()");
    expect(main).toContain("BackupBridge");
    expect(main).toContain("AvatarBridge");
    expect(main).toContain("_pickAvatarForWeb");
    expect(main).toContain("base64Encode");
    expect(main).toContain("window.setAvatarImageFromNative");
    ...
    expect(main).toContain("secureStorage.delete");
